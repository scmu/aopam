...
Recall that we aim to compute the minimum solutions under ordering |unlhd|.
Therefore, when |y0 `unlhd` y1|, we call |y0| the preferred solution, and |y1| the (possibly) nonpreferred one.
...
\end{equation}
On the lefthand side of \eqref{eq:monotonicity} we generate a pair of values |(y0, y1)| with the constraint that |y0 `unlhd` y1|.
Then we pair |y0| with one arbitrary result |z1|, generated by |f| from |y1|, the possibly less prefered candidate.
The inclusion says that
